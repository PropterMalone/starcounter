...
Investigation of existing Bluesky projects (`~/ergoblock`, `~/shared-kit`, `~/sample-app`) revealed consistent patterns:
...
- Vitest with 95% coverage target (shared-kit standard)
- Colocated tests
- Result type for expected failures (sample-app pattern)
- Rate limiting for Bluesky API (shared-kit implementation)
...
- Rate limiter pattern (adapt from shared-kit)
